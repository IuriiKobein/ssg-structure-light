...
cv::Mat alg_3dr_tpu_obj_depth_compute(sl_alg& alg, cxxopts::ParseResult& input) {
    ...
    return alg.depth_compute(lf_obj);
}
...
cv::Mat alg_3dr_pcg_obj_depth_compute(sl_alg& alg, cxxopts::ParseResult& input) {
    auto hf_obj = imgs_from_dir_load(input["hf_obj"].as<std::string>());

    return alg.depth_compute(hf_obj);
}

void alg_3dr_imgs_ref_compute(sl_alg& alg, cxxopts::ParseResult& input, int type) {
    if (type == TPU) {
...
cv::Mat alg_3dr_obj_depth_compute(sl_alg& alg, cxxopts::ParseResult& input, int type) {
    if (type == TPU) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        opt.add_options()("h,height", "images height", cxxopts::value<int>(h))(
            ...
            "method", "method for phase unwrap",
