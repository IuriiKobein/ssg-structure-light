...
class sl_pcg_alg::sl_alg_impl {
   public:
    ...
    int ref_phase_compute(const std::vector<cv::cuda::GpuMat>& refs) {
        cuda_phase_compute(refs, _temp_phases, _hf_ref_phase, *_filt);
        return 0;
    }

    int obj_phase_compute(const std::vector<cv::cuda::GpuMat>& objs) {
        cuda_phase_compute(objs, _temp_phases, _hf_obj_phase, *_filt);
    ...
    cv::Mat compute_3dr_impl() {
        return _cu_pu_alg.gradient_unwrap(_hf_obj_phase);
    }
