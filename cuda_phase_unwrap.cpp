...
cv::Scalar cuda_mean(cv::cuda::GpuMat &img) {
    return cv::Scalar(cv::cuda::sum(img)[0] / img.cols / img.rows);
}

cv::cuda::GpuMat &cuda_dct2(cv::cuda::GpuMat &img) {
    ...
    auto &carr = g_alg_env.t_mats->c_arr;
...
cv::cuda::GpuMat &cuda_idct2(cv::cuda::GpuMat &img) {
    auto &x = g_alg_env.t_mats->x;

    x.release();

    cv::cuda::transpose(idct(img), x);
    cv::cuda::transpose(idct(x), x);
...
cv::cuda::GpuMat &delta_phi(cv::cuda::GpuMat &img) {
